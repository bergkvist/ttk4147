...
char *counter_cmd = "stop\n";
int counter_val = 0;
void * counter(void * args) {
	for(EVER) {
		if (strcmp(counter_cmd, "up\n")) {
			counter_val++;
		} else if (strcmp(counter_cmd, "down\n")) {
			counter_val--;
		} else if (strcmp(counter_cmd, "stop\n")) {
...
int main(int argc, char *argv[]) {
	pthread_t handle;
	pthread_create(&handle, NULL, counter, NULL);
...
int io_write(resmgr_context_t *ctp, io_write_t *msg, RESMGR_OCB_T *ocb) {
    ...
    counter_cmd = (char *) malloc(msg->i.nbytes + 1);
    if (counter_cmd == NULL)
        return(ENOMEM);
    ...
    resmgr_msgread(ctp, counter_cmd, msg->i.nbytes, sizeof(msg->i));
    counter_cmd [msg->i.nbytes] = '\0'; /* just in case the text is not NULL terminated */
    printf ("Received %d bytes = '%s'\n", msg -> i.nbytes, counter_cmd);
